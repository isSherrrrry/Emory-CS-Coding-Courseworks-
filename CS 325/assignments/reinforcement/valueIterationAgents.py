...
class ValueIterationAgent(ValueEstimationAgent):
            ...
            for state in self.mdp.getStates():
                if not self.mdp.isTerminal(state):
                    ...
                    for action in self.mdp.getPossibleActions(state):
                        thisQ = self.computeQValueFromValues(state, action)
                        if thisQ > finalMax:
                            finalMax = thisQ
                    values[state] = finalMax
            for thisstate, thisvalue in values.items():
                self.values[thisstate] = thisvalue
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for newstate, probs in self.mdp.getTransitionStatesAndProbs(state, action):
            qValue += probs * (self.discount * self.values[newstate] + self.mdp.getReward(state, action, newstate))
        return qValue

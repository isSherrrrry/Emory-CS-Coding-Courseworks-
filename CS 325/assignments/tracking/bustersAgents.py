...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghostDistribution in livingGhostPositionDistributions:
            ghostPos.append(ghostDistribution.argMax())

        ghostDis = [(self.distancer.getDistance(pacmanPosition, p),p) for p in ghostPos]
        closestGhost = min(ghostDis)[1]

        moves = [(self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, move), closestGhost), move)
                 for move in legal]
        bestMove = min(moves)[0]
        actions = [move[1] for move in moves if move[0] == bestMove]
        return actions[0]

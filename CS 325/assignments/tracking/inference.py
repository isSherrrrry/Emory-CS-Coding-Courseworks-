        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.beliefs:
            if(self.beliefs[p] > 0):
                ...
                newDist = self.getPositionDistribution(gameState)
                for pos in self.legalPositions:
                    newBeliefs[pos] += self.beliefs[p] * newDist[pos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for p in range(self.numParticles):
            pos = self.legalPositions[p % len(self.legalPositions)]
    ...
    def observe(self, observation, gameState):
        ...
        for i in range(self.numParticles):
            if any(allPossible.values()):
                ...
                newParticles.append(random.choice(self.legalPositions))
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            particle = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            temp.append(util.sample(particle))
        self.particles = temp
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        thisbelief = util.Counter()
        for p in self.particles:
            thisbelief[p] += 1.0
        thisbelief.normalize()
        return thisbelief
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        catesianGrid = list(itertools.product(self.legalPositions, self.legalPositions))
        random.shuffle(catesianGrid)

        self.particles = []
        for p in range(self.numParticles):
            pos = catesianGrid[p % len(catesianGrid)]
            self.particles.append(pos)
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for p in self.particles:
            newPos, weight = list(p), 1.0
            for i in range(self.numGhosts):
                ghost = p[i]
                if noisyDistances[i] is not None:
                    weight *= emissionModels[i][util.manhattanDistance(pacmanPosition, ghost)]
                else:
                    newPos[i] = self.getJailPosition(i)
            allPossible[tuple(newPos)] += weight
